-- Fix foreign key relationships and add sample data for financial management (final)

-- Add missing foreign key constraints (ignore if they exist)
DO $$ 
BEGIN
    -- Add camera subscription constraints
    ...
    -- Add maintenance charge constraints
...
) AS v(organization_id, billing_name, customer_type, billing_address, status)
WHERE NOT EXISTS (SELECT 1 FROM billing_customers LIMIT 1);
...
) AS v(organization_id, category_name, category_type, description, default_amount, billing_frequency, is_mandatory, is_active)
WHERE NOT EXISTS (SELECT 1 FROM charge_categories LIMIT 1);
...
) AS v(organization_id, service_name, service_category, description, unit_type, default_rate, billing_model, is_active)
...
) AS v(organization_id, amenity_name, amenity_type, description, capacity, hourly_rate, advance_booking_days, max_booking_hours, is_active)
WHERE NOT EXISTS (SELECT 1 FROM amenities LIMIT 1);

-- Insert sample society units (using correct enum values)
...
SELECT * FROM (VALUES
    ...
    ('00000000-0000-0000-0000-000000000001', 'P-001', 'parking', 'Parking Management', NULL, 250.0, 2.00, 500.00, 0, 'occupied'),
    ('00000000-0000-0000-0000-000000000001', 'C-001', 'commercial', 'ABC Store', NULL, 400.0, 8.00, 3200.00, 0, 'occupied')
) AS v(organization_id, unit_number, unit_type, owner_name, tenant_name, area_sqft, monthly_rate_per_sqft, monthly_flat_rate, parking_slots, status)
WHERE NOT EXISTS (SELECT 1 FROM society_units LIMIT 1);
...
AND NOT EXISTS (SELECT 1 FROM maintenance_charges LIMIT 1);

-- Insert camera subscriptions (if cameras exist)
...
AND EXISTS (SELECT 1 FROM cameras)
AND NOT EXISTS (SELECT 1 FROM camera_subscriptions LIMIT 1)
LIMIT 5;
...
FROM billing_customers bc
WHERE bc.customer_type != 'advertiser'
AND NOT EXISTS (SELECT 1 FROM anpr_service_charges LIMIT 1);
...
FROM billing_customers bc
WHERE bc.customer_type = 'advertiser'
AND NOT EXISTS (SELECT 1 FROM advertisement_campaigns LIMIT 1);
