-- Phase 1: Financial Accounting Module Database Enhancement
...
CREATE TABLE public.amenity_bookings (
  ...
  total_charges NUMERIC GENERATED ALWAYS AS (duration_hours * hourly_rate) STORED,
...
CREATE TABLE public.revenue_breakdown (
  ...
  percentage NUMERIC GENERATED ALWAYS AS (CASE WHEN revenue_amount > 0 THEN ROUND((revenue_amount * 100.0) / NULLIF((SELECT SUM(revenue_amount) FROM revenue_breakdown WHERE report_id = revenue_breakdown.report_id), 0), 2) ELSE 0 END) STORED,
...
CREATE TABLE public.profit_loss_statements (
  ...
  profit_margin NUMERIC GENERATED ALWAYS AS (CASE WHEN total_revenue > 0 THEN ROUND((net_profit * 100.0) / total_revenue, 2) ELSE 0 END) STORED,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
