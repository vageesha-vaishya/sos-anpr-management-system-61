...
CREATE POLICY "Platform admins see all organizations" ON organizations
  FOR ALL USING (public.get_user_role(auth.uid()) = 'platform_admin');

CREATE POLICY "Franchise admins see their franchise and customers" ON organizations
  FOR ALL USING (
    public.get_user_role(auth.uid()) IN ('franchise_admin', 'franchise_user') AND
    (id = public.get_user_organization(auth.uid()) OR parent_id = public.get_user_organization(auth.uid()))
  );

CREATE POLICY "Customer users see only their organization" ON organizations
  FOR ALL USING (
    public.get_user_role(auth.uid()) IN ('customer_admin', 'customer_user') AND
    id = public.get_user_organization(auth.uid())
  );

-- RLS Policies for Profiles
CREATE POLICY "Platform admins see all profiles" ON profiles
  FOR ALL USING (public.get_user_role(auth.uid()) = 'platform_admin');
...
-- RLS Policies for Location hierarchy (inherit organization access)
CREATE POLICY "Location access by organization" ON locations
  FOR ALL USING (
    public.get_user_role(auth.uid()) = 'platform_admin' OR
    organization_id = public.get_user_organization(auth.uid()) OR
    organization_id IN (
      SELECT id FROM organizations 
      WHERE parent_id = public.get_user_organization(auth.uid())
    )
  );

CREATE POLICY "Building access via location" ON buildings
  FOR ALL USING (
    location_id IN (
...
CREATE POLICY "Gate access via building" ON entry_gates
  FOR ALL USING (
    building_id IN (
      SELECT b.id FROM buildings b
      JOIN locations l ON b.location_id = l.id
      WHERE public.get_user_role(auth.uid()) = 'platform_admin' OR
      l.organization_id = public.get_user_organization(auth.uid()) OR
      l.organization_id IN (
        SELECT id FROM organizations 
        WHERE parent_id = public.get_user_organization(auth.uid())
      )
    )
...
CREATE POLICY "Camera access via gate" ON cameras
  FOR ALL USING (
    entry_gate_id IN (
...
-- RLS Policies for ANPR data
CREATE POLICY "ANPR detection access by organization" ON anpr_detections
  FOR ALL USING (
    camera_id IN (
      SELECT c.id FROM cameras c
      JOIN entry_gates eg ON c.entry_gate_id = eg.id
      JOIN buildings b ON eg.building_id = b.id
      JOIN locations l ON b.location_id = l.id
      WHERE public.get_user_role(auth.uid()) = 'platform_admin' OR
      l.organization_id = public.get_user_organization(auth.uid()) OR
...
-- RLS Policies for Vehicle lists
CREATE POLICY "Vehicle whitelist access by organization" ON vehicle_whitelist
  FOR ALL USING (
    public.get_user_role(auth.uid()) = 'platform_admin' OR
    organization_id = public.get_user_organization(auth.uid()) OR
    organization_id IN (
...
CREATE POLICY "Vehicle blacklist access by organization" ON vehicle_blacklist
  FOR ALL USING (
    public.get_user_role(auth.uid()) = 'platform_admin' OR
    organization_id = public.get_user_organization(auth.uid()) OR
    organization_id IN (
...
CREATE POLICY "Alert access by organization" ON alerts
  FOR ALL USING (
    public.get_user_role(auth.uid()) = 'platform_admin' OR
    organization_id = public.get_user_organization(auth.uid()) OR
    organization_id IN (
