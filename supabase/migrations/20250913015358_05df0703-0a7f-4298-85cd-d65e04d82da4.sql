-- Complete Database Reset Script for Testing
...
-- Clean user-generated transactional data (in dependency order)
...
DELETE FROM password_change_audit;
DELETE FROM role_change_audit;
...
-- Clean user permissions and profiles (keep platform admin only)
DELETE FROM user_permissions WHERE user_id NOT IN (
  SELECT id FROM profiles WHERE email = 'bahuguna.vimal@gmail.com'
);
...
-- Test Organizations
INSERT INTO organizations (id, name, organization_type, status, created_at, updated_at) VALUES 
(gen_random_uuid(), 'Test Franchise Organization', 'franchise', 'active', now(), now()),
(gen_random_uuid(), 'Test Society Organization', 'customer', 'active', now(), now());
...
BEGIN
    ...
    -- Create test society units
    INSERT INTO society_units (id, unit_number, unit_type, floor_number, building_id, area_sqft, status, created_at, updated_at) VALUES
    (gen_random_uuid(), 'A-101', 'apartment', 1, test_building_id, 1200, 'occupied', now(), now()),
    (gen_random_uuid(), 'A-102', 'apartment', 1, test_building_id, 1200, 'occupied', now(), now()),
    (gen_random_uuid(), 'A-201', 'apartment', 2, test_building_id, 1500, 'vacant', now(), now()),
    (gen_random_uuid(), 'A-202', 'apartment', 2, test_building_id, 1500, 'occupied', now(), now());
...
COMMENT ON TABLE organizations IS 'Reset script executed - database cleaned for testing with basic test data structure';