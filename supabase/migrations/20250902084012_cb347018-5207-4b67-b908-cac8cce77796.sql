-- Fix the tickets table to use VARCHAR instead of enum for now
-- and create essential missing tables

-- Create essential missing tables for Phase 1 core features
...
-- Helpdesk System (using VARCHAR for priority instead of enum)
...
CREATE INDEX IF NOT EXISTS idx_visitors_organization ON public.visitors(organization_id);
...
CREATE INDEX IF NOT EXISTS idx_polls_organization ON public.polls(organization_id);

-- Create triggers for updated_at columns (if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_currencies_updated_at') THEN
