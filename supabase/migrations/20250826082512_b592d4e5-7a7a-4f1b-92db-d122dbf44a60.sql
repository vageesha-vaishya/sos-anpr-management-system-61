-- Fix foreign key relationships and add sample data for financial management (FINAL with all correct enums)
...
-- Insert additional ANPR service charges for subscription model
INSERT INTO anpr_service_charges (customer_id, service_type, charge_amount, unit_price, quantity, billing_date, billing_period_start, billing_period_end, status)
SELECT 
    bc.id,
...
AND NOT EXISTS (SELECT 1 FROM anpr_service_charges WHERE customer_id = bc.id AND service_type = 'subscription_professional');

-- Insert advertisement campaigns with correct enum values
...
-- Add another campaign type for variety
INSERT INTO advertisement_campaigns (organization_id, advertiser_id, campaign_name, campaign_type, billing_model, rate, daily_budget, total_budget, start_date, end_date, status)
SELECT 
    '00000000-0000-0000-0000-000000000001'::uuid,
    bc.id,
...
FROM billing_customers bc
WHERE bc.customer_type = 'advertiser'
AND NOT EXISTS (SELECT 1 FROM advertisement_campaigns WHERE advertiser_id = bc.id AND campaign_name = 'Digital Signage Campaign');
