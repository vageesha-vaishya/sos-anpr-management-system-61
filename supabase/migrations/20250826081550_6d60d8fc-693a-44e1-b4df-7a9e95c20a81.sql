-- Fix foreign key relationships and add sample data for financial management (fixed syntax)
...
FROM billing_customers bc
WHERE bc.customer_type != 'advertiser'
  AND NOT EXISTS (SELECT 1 FROM anpr_service_charges anpr WHERE anpr.customer_id = bc.id);
